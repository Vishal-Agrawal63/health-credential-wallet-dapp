...
const VerifyCredential = () => {
    ...
    // Helper to convert ipfs:// URI to a usable gateway URL
    ...
    // Helper to check if a credential has expired based on its date string
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // 1. Create a read-only provider to connect to Sepolia without MetaMask
            if (!sepoliaRpcUrl) {
            ...
            // 2. Verify token existence and get on-chain data
            // ownerOf() will throw an error if the token doesn't exist, which is a perfect check.
            ...
            // 4. Combine all data and set state
            setCredential({
                tokenId,
                ownerAddress,
    ...
    return (
        <div className="auth-wrapper">
            <div className="auth-container" style={{maxWidth: '600px'}}>
                ...
                {credential && (
                    <div className="credential-card" style={{marginTop: '2rem', textAlign: 'left'}}>
                        <div className="card-header">
                            Verification Result
                            {credential.isExpired && <span className="expired-badge">EXPIRED</span>}
                            {!credential.isExpired && <span className="valid-badge">VALID</span>}
                        </div>
                        <div className="card-body">
                             <div className="section-title">Document Info</div>
                             <div className="data-row">
                                <span className="icon">📎</span><span className="label">Title:</span>
                                <span className="value">{credential.metadata.title}</span>
                             </div>
                             <div className="data-row">
                                <span className="icon">🏥</span><span className="label">Issuer:</span>
                                <span className="value">{credential.metadata.issuer}</span>
                             </div>
                             <div className="data-row">
                                <span className="icon">🗓️</span><span className="label">Issued:</span>
                                <span className="value">{new Date(credential.metadata.issuedDate).toLocaleDateString()}</span>
                             </div>
                            {credential.metadata.expiryDate && (
                                <div className="data-row">
                                    <span className="icon">⏳</span><span className="label">Expires:</span>
                                    <span className={`value ${credential.isExpired ? 'expired-text' : ''}`}>{new Date(credential.metadata.expiryDate).toLocaleDateString()}</span>
                                </div>
                            )}
                            <hr />
                            <div className="section-title">Blockchain Data</div>
                            <div className="data-row">
                                <span className="icon">📦</span><span className="label">Token ID:</span>
                                <span className="value">#{credential.tokenId}</span>
                             </div>
                            <div className="data-row">
                                <span className="icon">👤</span><span className="label">Owner:</span>
                                <span className="value">{truncateHash(credential.ownerAddress)}</span>
                             </div>
                            <div className="data-row">
                                <span className="icon">🔗</span><span className="label">Contract:</span>
                                <span className="value">{truncateHash(contractAddress)}</span>
                             </div>
                        </div>
